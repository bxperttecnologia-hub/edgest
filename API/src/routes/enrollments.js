...
// POST /api/enrollments  (faz inscrição e gera parcelas na tabela payments)
router.post('/add', async (req, res) => {
  try {
    const { student_id, class_id, enrollment_date, selected_parcela } = req.body;

    const rows = await query(
      'CALL sp_add_enrollment(?, ?, ?, ?)',
      [
        student_id,
        class_id,
...
router.post('/addTeacher', async (req, res) => {
  try {
    ...
    const existing = await query("SELECT * FROM instructors WHERE email = ? ", [email]);

    if (existing.length > 0) {
      res.status(401).json({ message: "Já existe um instrutor cadastrado com este email!" })
    } else {


      const result = await query("INSERT INTO instructors(name, email, contact) VALUES(?, ?, ?)", [
        name, email, contact
      ])
...
router.post("/addClass", async (req, res) => {
  try {
    ...
    if (!course_id && !instructor_id && !capacity && !start_date && !end_date && !name, !schedule) {
      res.status(401).json({ message: "Preencha todos os campos!" })
    } else {

      const lastId = await getLastId("classes", "id");

      const existis = await query("SELECT * FROM classes WHERE course_id = ?", [name]);
      let code = "";
      ...
      const result = await pool.query("INSERT INTO classes (course_id, name, start_date, end_date, capacity, instructor_id, schedule) VALUES(?, ?, ?, ?, ?, ?, ?)", [
        course_id,
        code,
        start_date,
        end_date,
...
router.post("/studentEnrollment/:id", async (req, res) => {
  try {

    const { id } = req.params;

    const sqlSelect = `SELECT e.id, c.code, c.title as course, e.created_at FROM enrollments e JOIN users u ON u.id = e.student_id JOIN classes cl ON cl.id = e.class_id JOIN courses c ON c.id = cl.course_id WHERE u.id = ?`;
...
router.delete("/delete-student-enrollment/:id", async (req, res) => {
  try {
    ...
    const sqlSelect = `DELETE FROM enrollments WHERE id = ?`;
    const result = await query(sqlSelect, [id]);

    if(result.affectedRows > 0){
      res.status(200).json({message: "Inscrição deletado com sucesso!"})
    }else{
      res.status(404).json([])
    }
